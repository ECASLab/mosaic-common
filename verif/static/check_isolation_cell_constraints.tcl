if {$argc != 1} {
    puts stderr "usage: check_isolation_cell_constraints.tcl SDC"
    exit 2
}
...
proc record_singleton {name value} {
    global observations
    if {[dict exists $observations $name]} { error "$name appears more than once" }
...
foreach {field expected} {
    ...
    if {![dict exists $observations $field] || [dict get $observations $field] ne $expected} {
        error "isolation-cell SDC does not define the reviewed $field intent"
    }
}
foreach forbidden {forbidden_clock forbidden_false_path forbidden_clock_groups} {
    if {[dict exists $observations $forbidden]} {
        error "isolation-cell SDC contains forbidden clock or exception intent"
    }
}
puts "Isolation-cell combinational constraint intent passed"
