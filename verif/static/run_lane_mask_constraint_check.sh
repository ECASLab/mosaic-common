...
if [[ "${SYNTHESIS_CONSTRAINT_FILE}" != "${CONSTRAINT_DIR}/lane_mask.timing.sdc" ]]; then
    echo "The synthesis flow selected an unexpected lane-mask SDC" >&2
    exit 1
fi
...
"${TCLSH_CMD:-tclsh}" verif/static/check_lane_mask_constraints.tcl \
