...
# or broad timing exceptions that would hide interface paths.
if {$argc != 1} {
    puts stderr "usage: check_lane_mask_constraints.tcl SDC"
    exit 2
}
...
foreach {field expected} {
    ...
    if {![dict exists $observations $field]} {
        error "lane-mask SDC does not define $field"
    }
    if {[dict get $observations $field] ne $expected} {
        error "lane-mask SDC $field does not match the reviewed interface intent"
    }
}

foreach forbidden {forbidden_clock forbidden_false_path forbidden_clock_groups} {
    if {[dict exists $observations $forbidden]} {
        error "lane-mask SDC contains forbidden clock or exception intent"
    }
}

puts "Lane-mask combinational constraint intent passed"
