if {$argc != 1} {
    puts stderr "usage: check_level_shifter_constraints.tcl SDC"
    exit 2
}
...
foreach {field expected} {
    ...
    if {![dict exists $observations $field]} {
        error "level-shifter SDC does not define $field"
    }
    if {[dict get $observations $field] ne $expected} {
        error "level-shifter SDC $field does not match the reviewed portable intent"
    }
}

foreach forbidden {forbidden_clock forbidden_false_path forbidden_clock_groups} {
    if {[dict exists $observations $forbidden]} {
        error "level-shifter SDC contains forbidden clock or exception intent"
    }
}

puts "Level-shifter combinational constraint intent passed"
