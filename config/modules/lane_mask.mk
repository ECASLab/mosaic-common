# Lane-mask tops and design-owned flow inputs.
export DESIGN_TOP := lane_mask
export TB_TOP := $(DESIGN_TOP)_tb
export FORMAL_TOP := $(DESIGN_TOP)_formal
export DUT_INSTANCE := $(TB_TOP)/checker_lanes_4/dut
...
export ASSERTION_COVERPOINT_REQUIREMENTS := operation_disabled_covered=5 all_lanes_active_covered=5 lane_valid_suppression_covered=5 architectural_mask_suppression_covered=5 predicate_suppression_covered=5
export ASSERTION_COVERAGE_EXCLUDED_RTL_LINES := 17
export FORMAL_COVERAGE_CONFIG :=
export CONSTRAINT_CHECK_SCRIPT := $(MODULE_ROOT)/verif/static/run_lane_mask_constraint_check.sh
export FAULT_INJECTION_SCRIPT := $(MODULE_ROOT)/verif/tb/fault_injection/run_lane_mask_fault_injection.sh
export FOUR_STATE_CHECK_SCRIPT := $(MODULE_ROOT)/verif/tb/four_state/run_lane_mask_four_state.sh
