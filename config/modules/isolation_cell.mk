# Isolation-cell tops and design-owned flow inputs.
export DESIGN_TOP := isolation_cell
export TB_TOP := $(DESIGN_TOP)_tb
export FORMAL_TOP := $(DESIGN_TOP)_formal
export DUT_INSTANCE := $(TB_TOP)/c04/dut
...
export ASSERTION_COVERPOINT_REQUIREMENTS := isolation_active_covered=16 isolation_inactive_covered=16 clamp_differs_covered=32
# The invalid elaboration branch and the inactive side of each static polarity
# choice are structurally unreachable in a valid elaborated instance.
export ASSERTION_COVERAGE_EXCLUDED_RTL_LINES := 14 17 20
export FORMAL_COVERAGE_CONFIG :=
export CONSTRAINT_CHECK_SCRIPT := $(MODULE_ROOT)/verif/static/run_isolation_cell_constraint_check.sh
export FAULT_INJECTION_SCRIPT := $(MODULE_ROOT)/verif/tb/fault_injection/run_isolation_cell_fault_injection.sh
export FOUR_STATE_CHECK_SCRIPT := $(MODULE_ROOT)/verif/tb/four_state/run_isolation_cell_four_state.sh
